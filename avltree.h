...
private:
    ...
    public:
        ...
        AvlTreeNode(const TKey& k,const TValue& val){
            value=val;
            key=k;
            bf=0;
            left=right=nullptr;
    ...
    //左旋函数，详细说明见定义
    static void left_rotate(AvlTreeNode** node, unsigned char* rt);

    //右旋函数，详细说明见定义
    static void right_rotate(AvlTreeNode** node, unsigned char* rt);
...
//rt:记录旋转情况的标记位
template<class TKey,class TValue>
void AvlTree<TKey,TValue>::left_rotate(AvlTreeNode **node, unsigned char *rt) {
...
//rt:记录旋转情况的标记位
template<class TKey,class TValue>
void AvlTree<TKey,TValue>::right_rotate(AvlTreeNode **node, unsigned char *rt) {
    AvlTreeNode* son,* pson;
    son = (*node)->left;
